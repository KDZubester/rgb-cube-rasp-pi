...
void updateSnake() 
{
  ...
  if (snakeMovementCounter > maxSnakeCounter) 
  {
    ...
    if (snake.x > 3) {snake.x = 0;}
    if (snake.x < 0) {snake.x = 3;}
    if (snake.y > 3) {snake.y = 0;}
    if (snake.y < 0) {snake.y = 3;}
    if (snake.z > 3) {snake.z = 0;}
    if (snake.z < 0) {snake.z = 3;}
    ...
    for (int i = 0; i < s_length; i++) 
    {
      setLED(tail[i].x, tail[i].y, tail[i].z, 0, 255, 0);
    }

    // Draw snake head
    setLED(snake.x, snake.y, snake.z, 0, 255, 0);
  }
...
void updateApple() 
{
  ...
  else { setLED(apple.x2, apple.y2, apple.z2, 255, 0, 0); }
}
...
void checkCollisions() 
{
  ...
  if ((snake.x == apple.x2) && (snake.y == apple.y2) && (snake.z == apple.z2)) 
  {
    ...
    genNewApple();
    // Update the score on the tft screens
    p1_score++;
    updateScore(p1_score, 0, 0, 0, &tft_1);
    // updateScore(p1_score, 0, 0, 0, &tft_2);
  }
